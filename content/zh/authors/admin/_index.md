---
title: 刘书宁
name_pronunciation: Shuning Liu
first_name: Shuning
last_name: Liu
status:
  icon: 🔬
superuser: true
highlight_name: true
role: 微电子学博士研究生
organizations:
  - name: 香港科技大学（广州）
    url: https://www.hkust-gz.edu.cn/
profiles:
  - icon: at-symbol
    url: 'mailto:liushuning211@mails.ucas.ac.cn'
    label: 电子邮件
  - icon: academicons/google-scholar
    url: https://scholar.google.com/
    label: Google Scholar
  - icon: academicons/orcid
    url: https://orcid.org/
    label: ORCID
  - icon: brands/linkedin
    url: www.linkedin.com/in/shuning-liu-9b8876291
    label: LinkedIn
  - icon: brands/github
    url: https://github.com/Estel-lab
    label: GitHub
interests:
  - 红外光电探测器
  - 雪崩光电二极管 (APD)
  - 碲镉汞 (HgCdTe) 器件
  - 蒙特卡洛器件仿真
  - 光电逻辑器件
  - 二维材料与异质结
education:
  - area: 微电子学博士
    institution: 香港科技大学（广州）
    date_start: 2024-09-01
    date_end: 2027-09-01
    summary: |
      研究方向：窄带隙半导体雪崩光电二极管、介观载流子输运、器件物理仿真。导师：胡伟达教授。
  - area: 微电子学与固体电子学硕士
    institution: 中国科学院上海技术物理研究所
    date_start: 2021-09-01
    date_end: 2024-09-01
    summary: |
      导师：胡伟达研究员。在 Sci. China: Phys. Mech. Astron.、Infrared Phys. Technol.、Infrared Laser Eng. 等期刊以第一/共同第一作者发表论文3篇。在 Nat. Photonics、IEEE J. Quantum Electron.、Advanced Optical Materials 等期刊合作发表论文5篇以上。
      
      课程：半导体物理前沿、智能感知、探测系统理论、智能传感器与信息处理、半导体微纳加工。
  - area: 材料科学与工程学士
    institution: 山东大学
    date_start: 2017-07-01
    date_end: 2021-09-01
    summary: |
      课程：材料物理、材料科学基础、量子力学、固体物理、高等数学。
      毕业论文：模拟STM图像中原子结构的模板匹配识别。
work:
  - position: 博士研究员
    company_name: 香港科技大学（广州）
    company_url: https://www.hkust-gz.edu.cn/
    date_start: 2024-09-01
    date_end: ''
    summary: |
      - 开发窄带隙半导体雪崩过程的介观蒙特卡洛仿真框架
      - 研究超低噪声碲镉汞雪崩光电二极管的电场调控策略
      - 研究红外APD中载流子输运动力学与过剩噪声机制
  - position: 硕士研究员
    company_name: 中国科学院上海技术物理研究所
    date_start: 2021-09-01
    date_end: 2024-09-01
    summary: |
      - 构建碲镉汞电子雪崩光电二极管微观-介观蒙特卡洛范式用于过剩噪声分析
      - 改进稀疏采样算法以提升器件仿真计算效率
      - 参与多个红外探测器设计与表征项目
skills:
  - name: 编程与仿真
    items:
      - name: C/C++
        description: '蒙特卡洛器件仿真'
        percent: 90
        icon: code-bracket
      - name: Python
        description: '数据分析与深度学习'
        percent: 85
        icon: code-bracket
      - name: MATLAB
        description: '科学计算'
        percent: 85
        icon: chart-bar
      - name: SQLite
        description: '仿真数据数据库管理'
        percent: 75
        icon: circle-stack
  - name: 高性能计算与基础设施
    color: '#3b82f6'
    items:
      - name: MPI
        description: '并行计算'
        percent: 80
        icon: server
      - name: OpenMP
        description: '共享内存并行化'
        percent: 75
        icon: server
      - name: 深度学习框架
        description: 'PyTorch，模型训练与部署'
        percent: 70
        icon: cpu-chip
  - name: 实验技能
    color: '#10b981'
    items:
      - name: LabVIEW
        description: '数据采集与处理'
        percent: 80
        icon: signal
      - name: 光学系统搭建
        description: '光路构建与对准'
        percent: 75
        icon: light-bulb
languages:
  - name: 中文
    percent: 100
  - name: 英语
    percent: 85
awards:
  - title: 中国科学院大学优秀毕业生
    date: '2024-06-01'
    awarder: 中国科学院大学
    icon: academic-cap
  - title: 中国科学院大学三好学生
    date: '2024-06-01'
    awarder: 中国科学院大学
    icon: academic-cap
  - title: 中国科学院大学三好学生
    date: '2023-06-01'
    awarder: 中国科学院大学
    icon: academic-cap
  - title: 中国科学院大学三好学生
    date: '2022-06-01'
    awarder: 中国科学院大学
    icon: academic-cap
  - title: 学业奖学金（二等）
    date: '2019-06-01'
    awarder: 山东大学
    icon: academic-cap
  - title: 优秀学生干部
    date: '2019-06-01'
    awarder: 山东大学
    icon: star
  - title: 学业奖学金（二等）
    date: '2018-06-01'
    awarder: 山东大学
    icon: academic-cap
---

## 关于我

刘书宁，香港科技大学（广州）微电子学博士研究生，拥有中国科学院上海技术物理研究所硕士学位。研究方向为红外光电探测器，尤其是窄带隙半导体雪崩光电二极管（HgCdTe APD）、介观载流子输运动力学及器件物理仿真。

以第一/共同第一作者在 *Science China: Physics, Mechanics & Astronomy*、*Infrared Physics & Technology*、*Infrared and Laser Engineering* 等期刊发表多篇论文，并在 *Nature Photonics*、*IEEE Journal of Quantum Electronics*、*Advanced Optical Materials*、*Optics Express* 等期刊合作发表论文。

当前主要从事微观-介观蒙特卡洛框架的开发，用于理解雪崩载流子动力学，并通过电场调控策略设计超低噪声红外光电探测器。
